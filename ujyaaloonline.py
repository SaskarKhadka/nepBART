...
for category, category_details in UJYAALOONLINE_WEBSITES.items():
    for page in range(1, category_details[1]):
        res = req.get(category_details[0] + ("" if page == 1 else f"?page={page}"), headers=headers)
        if res.status_code == 200:
            soup = BeautifulSoup(res.content, "html5lib")
            titles_info = soup.select("div.grid-posts div.col-md-4.padding-10 div.grid-post")
            for title_info in titles_info:
                news = title_info.select("div.post-info.padding-15.ptb-20.bd-grey.match-height.text-center h3.mb-15.fw-6.fz-20 a.text-dark")[0]
