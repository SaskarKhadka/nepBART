...
emountain_data_path = os.path.join(data_path, "emountain")

if not os.path.exists(data_path):
    os.mkdir(data_path)

if not os.path.exists(emountain_data_path):
    os.mkdir(emountain_data_path)

total_existsing_news = len(os.listdir(emountain_data_path))
...
for category, category_details in EMOUNTAIN_WEBSITES.items():
    for page in range(1, category_details[1]):
        res = req.get(
            category_details[0] + ("" if page == 1 else f"/page/{page}"),
            headers=headers,
        )
        with open("emountain_page.json", "w") as file:
        ...
        if res.status_code == 200:
            ...
            titles_info = soup.select(
                "div#content div.col-md-3 div.card-body.px-0.pb-3.pt-5 h5.header-primary "
            )
            for title_info in titles_info:
                ...
                if news_text.strip() != "":
                    ...
                    ).to_csv(os.path.join(emountain_data_path, f"{news_count}.csv"))
                    news_count += 1
                    time.sleep(4)
